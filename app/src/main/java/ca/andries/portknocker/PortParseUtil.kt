...
class PortParseUtil {
    companion object {
        fun validateAndParsePorts(context: Context, content: String) : List<Int> {
            ...
            for (portInput in portInputs) {
                ...
                val parsedPort = try {
                    ...
                    throw PortKnockerException(context.getString(R.string.bad_port, portInput))
                }
                result.add(parsedPort)
            }
            if (result.isEmpty()) {
                throw PortKnockerException(context.getString(R.string.no_ports_specified))
            }
            return result
        }
        ...
        fun validateAndParseOneTimeSequences(context: Context, content: String) : ArrayList<List<Int>> {
            ...
            return ArrayList(lines.map { line -> validateAndParsePorts(context, line) })
        }
    }
}